...
def to_base64(type_name: str, node_id: Any) -> str:
...
class GlobalID:
    ...
    def resolve_node(self, info, node_type=None):
        ...
        if node_type is not None:
            return aio.resolve(node, lambda n: n, info=info, ensure_type=node_type)
...
class Node(Generic[NodeType]):
    ...
    def id(cls, info: Info, root: "Node[NodeType]") -> GlobalID:  # noqa:A003
        ...
        if isinstance(node_id, str):
            ...
            return aio.resolve_async(  # type:ignore
                node_id,
                lambda v: GlobalID(type_name=type_name, node_id=v),
            )
    ...
    @classmethod
    def resolve_nodes_resolver(
        cls,
        info: Info,
        resolver: Callable[[Iterable[NodeType], Optional[int]], _T],
        *,
        nodes: Optional[AwaitableOrValue[Iterable[NodeType]]] = None,
    ) -> AwaitableOrValue[_T]:
        if nodes is None:
            nodes = cls.resolve_nodes(info)
        return aio.resolve(nodes, lambda n: resolver(n, None), info=info)
...
class NodeField(StrawberryField):
    ...
    def __call__(self, resolver):
        if self.node_type is not None and not isinstance(resolver, StrawberryResolver):
            resolver.__annotations__["return"] = self.node_type
...
class ConnectionField(StrawberryField):
    def __init__(self, *args, **kwargs):
        ...
        self.connection_resolver = StrawberryResolver(self.default_resolver)
        if not self.base_resolver:
    ...
    def get_result(
        ...
        return field_type.resolve_nodes_resolver(info, resolver, nodes=nodes)
